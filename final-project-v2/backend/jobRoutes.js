...
router.get('/getjob', async (req, res) => {
    try {
        ...
        const page = parseInt(req.query.page) || 1;      // 当前页数
        const limit = 10;                                  // 每页返回 8 个
        const skip = (page - 1) * limit;                  // 跳过的数量
    
        const total = await Job.countDocuments();         // 总条数
        const jobs = await Job.find().skip(skip).limit(limit); // 分页查询
        
        if (jobs.length === 0) {
            return res.status(404).json({ error: 'No Question found' });
        }

        res.status(200).json({jobs, total});  // 返回职位数据
    } catch (error) {
